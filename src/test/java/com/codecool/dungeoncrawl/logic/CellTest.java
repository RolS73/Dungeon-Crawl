...
import com.codecool.dungeoncrawl.logic.actors.items.looting.Key;
...
class CellTest {
    ...
    void cellSetters() {
        ...
        assertEquals(Player.class ,map.getCell(0,0).getActor().getClass());
        Key key = new Key(map.getCell(2,2));
        map.getCell(2,2).setItem(key);
        assertEquals(Key.class, map.getCell(2,2).getItem().getClass());
        map.getCell(2,0).setActor(key);
        assertEquals(Key.class, map.getCell(2,0).getItem().getClass());
